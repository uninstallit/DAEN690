...
from functions_.functions import get_quadruplet_index  # get_triplet_index_from_good_bad
...
def main():
    ...
    negative_one_series_list = [
        notams_dict[notam_rec_id] for notam_rec_id in negative_one_index
    ]
    negative_one_df = pd.concat(negative_one_series_list, axis=1).transpose()

    negative_two_series_list = [
        notams_dict[notam_rec_id] for notam_rec_id in negative_two_index
    ]
    negative_two_df = pd.concat(negative_two_series_list, axis=1).transpose()
    ...
    features_pipeline.fit_transform(notams_df)
    ...
    np.save("./data/negative_two_data", negative_one_data)
