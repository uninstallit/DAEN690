...
def semantic_search_similar_notams(queries_df, corpus_df):
    ...
    for i in queries_df.index:
        ...
        query_embedding = embedder.encode(q_notam_txt, convert_to_numpy=True)
        ...
        for score, idx in zip(top_results[0], top_results[1]):
            print(
                f"\n notam_rec_id:{notam_rec_ids[idx]}",
                "%.110s..." % corpus[idx],
...
def main():
    ...
    sql = """ SELECT NOTAM_REC_ID, E_CODE, TEXT FROM notams WHERE 
                DATETIME(POSSIBLE_START_DATE) < '2018-04-02 20:30:38' and DATETIME(POSSIBLE_END_DATE) = '2018-04-02 21:08:00' 
                and (E_CODE not null or TEXT not null) """
    data = cursor.execute(sql).fetchall()
    # take E code col if not take Text column
    notam_df = pd.DataFrame(
        {
            "NOTAM_REC_ID": [d[0] for d in data],
            "TEXT": [d[1] if d[1] else d[2] for d in data],
        }
    )
    ...
    semantic_search_similar_notams(query_df, corpus_df)
